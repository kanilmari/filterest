...
| P0 | Third-party notices | blocked-evidence | Release agent + human/project owner | Regenerate `THIRD_PARTY_NOTICES.md` and `THIRD_PARTY_LICENSES/` with complete retained license and notice texts, then obtain owner acceptance for their exact final bytes. The generated `THIRD_PARTY_NOTICE_REVIEW.md` must verify dependency identity, scope, asset provenance, and every bundled text hash. |

...
| --- | --- | --- | --- | --- |
| P0 | Final source license chosen | done | Human release owner | Owner confirmed on 2026-08-29 that Filterest uses GPL version 2 or later / `GPL-2.0-or-later` for its canonical public source, allowing GPLv3 distribution with Apache-2.0 components. |
| P0 | Final `LICENSE` file present | done | Human release owner + release agent | `LICENSE` contains the GNU General Public License version 2 text plus the later-version grant, and package metadata declares `GPL-2.0-or-later`; verify both after every candidate regeneration. |
...
| P0 | Third-party notices | done | Release agent + human/project owner | Owner accepted the generated notice SHA-256 `b0bdad9632163da49fa5e78bc253b9669b319f936830afffdd688c58ac1a0b88` and manifest SHA-256 `f09c60a3a24440e9d3431a4c78cb651a1e072dc4417052771ec8f4b2146bc1bb` on 2026-08-30 after the complete dependency, binary, browser-build, asset-provenance, and retained-text audit passed. Final readiness must fail if either byte sequence changes. |
...
| P1 | Public screenshots/demo data | done | Release agent | Auth-tour screenshots are intentionally excluded until replacement content and provenance are approved. [`app/server_tools/public_bootstrap/DEMO_ASSET_REVIEW.md`](../../server_tools/public_bootstrap/DEMO_ASSET_REVIEW.md) verifies that none are distributed and that all 15 immutable bootstrap fixture sources remain reviewed; first-run setup materializes runtime copies under the mutable data root. |

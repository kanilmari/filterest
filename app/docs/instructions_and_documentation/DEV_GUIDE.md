...
- **Routing**: Register core handlers in `app/backend/core_components/router/router.go` with a security profile in `app/backend/pipeline/route_profiles.go`. Optional application compositions use the existing `app_registry.RegisterRoute` and `pipeline.RegisterRouteProfile` extension points. See [Pipeline_Architecture.md](Pipeline_Architecture.md).

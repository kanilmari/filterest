...
| 5 | `app/frontend/core_components/admin_tools/main/oid_updater.js` | `fetch('/api/update-oids')` | Best-effort admin maintenance refresh. Uses a local `AbortController` timeout so a slow OID/catalog sync cannot keep reloads open for tens of seconds. | 2026-04-26 |

...
func TestLoadAndSetEnvironmentVariablesFromRootIgnoresWorkingDirectory(t *testing.T) {
	originalWd, err := os.Getwd()
	if err != nil {
		t.Fatalf("getwd: %v", err)
	}
	...
	if err := os.WriteFile(
		filepath.Join(installationRoot, ".env"),
		[]byte("ENVIRONMENT_TYPE=dev\n"),
		0o600,
	); err != nil {
		t.Fatalf("write installation .env: %v", err)
	}
	if err := os.WriteFile(
	...
	envType, err := loadAndSetEnvironmentVariablesFromRoot(installationRoot)
	if err != nil {
		t.Fatalf("loadAndSetEnvironmentVariablesFromRoot() error = %v", err)
	}
	if envType != "dev" {
		t.Fatalf("envType = %q, want installation-root value dev", envType)
	}
}
...
func TestResolveProjectPrivatePathsUsesNestedInstallationKeys(t *testing.T) {
	projectRoot := t.TempDir()
	...
	profileRoot := filepath.Join(projectRoot, "keys", "filterest_runtime")
	if len(envFiles) != 2 ||
		...
		t.Fatalf("nested env files = %#v", envFiles)
	}
	if tlsCertFile != filepath.Join(profileRoot, "local_tls_certificate", "localhost_certificate.crt") ||
		tlsKeyFile != filepath.Join(profileRoot, "local_tls_certificate", "localhost_private_key.key") {
		t.Fatalf("nested TLS paths = %q, %q", tlsCertFile, tlsKeyFile)
	}
}

...
func TestDetectFromWorkingDirectoryUsesConfiguredApplicationRoot(t *testing.T) {
	root := t.TempDir()
	if err := os.WriteFile(filepath.Join(root, "VERSION_APP"), []byte("8.0.17\n"), 0o644); err != nil {
		t.Fatal(err)
	}

	runtimeRootMu.Lock()
	previousRoot := runtimeApplicationRoot
	runtimeApplicationRoot = ""
	runtimeRootMu.Unlock()
	t.Cleanup(func() {
		runtimeRootMu.Lock()
		runtimeApplicationRoot = previousRoot
		runtimeRootMu.Unlock()
	})

	if err := ConfigureApplicationRoot(root); err != nil {
		t.Fatal(err)
	}
	identity := DetectFromWorkingDirectory()
	if identity.Kind != KindFilterestPublic || identity.Version != "8.0.17" {
		t.Fatalf("configured application identity = %+v", identity)
	}
}

...
var (
	...
	runtimeApplicationRoot    string
)

// ConfigureApplicationRoot records the immutable app directory selected at startup.
// Between the root resolver and request-time identity handlers, it prevents product
// identity from depending on the process working directory or executable location.
func ConfigureApplicationRoot(root string) error {
	root = strings.TrimSpace(root)
	if root == "" {
		return fmt.Errorf("Filterest application root is required")
	}
	absoluteRoot, err := filepath.Abs(root)
	if err != nil {
		return fmt.Errorf("resolve Filterest application root: %w", err)
	}
	info, err := os.Stat(absoluteRoot)
	if err != nil {
		return fmt.Errorf("inspect Filterest application root: %w", err)
	}
	if !info.IsDir() {
		return fmt.Errorf("Filterest application root is not a directory: %s", absoluteRoot)
	}

	runtimeRootMu.Lock()
	runtimeApplicationRoot = filepath.Clean(absoluteRoot)
...
func DetectFromWorkingDirectory() Identity {
	runtimeRootMu.RLock()
	root := runtimeApplicationRoot
	runtimeRootMu.RUnlock()
	if root != "" {
		return Detect(root)
	}
	return Detect("")

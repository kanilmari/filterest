...
func buildOrderByClause(
	...
	if sortColumn == semanticNewestSortColumn {
		return buildSemanticNewestOrderByClause(tableName, columnsByName, sortOrder), nil
	}
...
func buildSemanticNewestOrderByClause(
	tableName string,
	columnsByName map[string]dtt_models.ColumnInfo,
	sortOrder string,
) string {
	newestColumn := ""
	for _, candidate := range []string{"created", "created_at", "applied_at", "id"} {
		if _, exists := columnsByName[candidate]; exists {
			newestColumn = candidate
			break
		}
	}
	if newestColumn == "" {
	...
	orderByParts := []string{
		fmt.Sprintf(
			"%s.%s %s NULLS LAST",
			quotedTable,
			pq.QuoteIdentifier(newestColumn),
			sortOrder,
		),
	}
	if newestColumn != "id" {
		if _, hasIDColumn := columnsByName["id"]; hasIDColumn {
			orderByParts = append(
				orderByParts,
				fmt.Sprintf("%s.%s %s", quotedTable, pq.QuoteIdentifier("id"), sortOrder),
			)
		}
	}

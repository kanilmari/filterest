...
func TestBuildQueenSessionCommandUsesPortableDefaultModule(t *testing.T) {
	t.Setenv("FILTEREST_QUEEN_PYTHON_MODULE", "")
	command := buildQueenSessionCommand(context.Background(), queenManagedSessionCommandSpec{
		Prompt:         "Inspect the public checkout",
		MaxTurns:       3,
		ProjectRoot:    "/tmp/filterest",
		TranscriptPath: "/tmp/filterest/.queen/transcript.jsonl",
	})

	wantPrefix := []string{"python3", "-m", "server_tools.queen", "run"}
	...
	for index, want := range wantPrefix {
		if command.Args[index] != want {
			t.Fatalf("command arg %d = %q, want %q", index, command.Args[index], want)
		}
	}
	if command.Dir != "/tmp/filterest" {
		t.Fatalf("command dir = %q, want /tmp/filterest", command.Dir)
	}
}

func TestBuildQueenSessionCommandAcceptsEmbeddedModuleOverride(t *testing.T) {
	t.Setenv("FILTEREST_QUEEN_PYTHON_MODULE", "filterest.app.server_tools.queen")
	command := buildQueenSessionCommand(context.Background(), queenManagedSessionCommandSpec{
		Prompt:         "Inspect the embedded checkout",
		MaxTurns:       3,
		ProjectRoot:    "/tmp/easelect",
		TranscriptPath: "/tmp/easelect/.queen/transcript.jsonl",
	})

	if got := command.Args[2]; got != "filterest.app.server_tools.queen" {
		t.Fatalf("queen module = %q, want filterest.app.server_tools.queen", got)
	}
}

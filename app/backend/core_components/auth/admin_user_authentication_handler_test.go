...
func TestAdminUserAuthenticationUsesCurrentSingleChallengeSchema(t *testing.T) {
	source, err := os.ReadFile("admin_user_authentication_handler.go")
	if err != nil {
		t.Fatalf("read handler source: %v", err)
	}
	if strings.Contains(string(source), "used IS FALSE") {
		t.Fatal("administrator provisioning must delete stored challenges without the removed used column")
	}
}

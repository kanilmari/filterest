...
const translationSystemMessageRelativePath = "docs/instructions_and_documentation/ai_instructions/ai_db_cell_translations.md"
...
func translateText(ctx context.Context, text, column string) (translationResult, error) {
	sysBytes, err := readTranslationSystemMessage()
	if err != nil {
...
// readTranslationSystemMessage locates the public prompt in standalone and transition source layouts.
// It bridges a Filterest repository root and the outer Easelect checkout that owns filterest/.
// This keeps one canonical prompt file while both runtime layouts remain usable during migration.
func readTranslationSystemMessage() ([]byte, error) {
	candidates := []string{
		translationSystemMessageRelativePath,
		"filterest/" + translationSystemMessageRelativePath,
	}
	...
	for _, candidate := range candidates {
		...
		if !errors.Is(err, os.ErrNotExist) {
			return nil, fmt.Errorf("read AI translation system message %q: %w", candidate, err)
		}
		lastErr = err
	}

	return nil, fmt.Errorf(
		"read AI translation system message from %s or %s: %w",
		candidates[0],
		candidates[1],
		lastErr,
	)

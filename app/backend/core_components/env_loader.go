...
func LoadEnvironmentVariables() (string, error) {
	...
	return LoadEnvironmentVariablesFromRoot(projectRoot)
}

// LoadEnvironmentVariablesFromRoot loads the runtime environment relative to
// one explicit installation root. Keeping this boundary separate from the
// immutable application directory lets a packaged app/ remain read-only while
// its keys and operator configuration live beside it.
func LoadEnvironmentVariablesFromRoot(projectRoot string) (string, error) {
	envLoadMu.Lock()
	defer envLoadMu.Unlock()

	envType, err := loadAndSetEnvironmentVariablesFromRoot(projectRoot)
	envLoaded = true
...
func loadAndSetEnvironmentVariables() (string, error) {
	...
	return loadAndSetEnvironmentVariablesFromRoot(projectRoot)
}

func loadAndSetEnvironmentVariablesFromRoot(projectRoot string) (string, error) {
	absoluteRoot, err := filepath.Abs(projectRoot)
	if err != nil {
		return "", fmt.Errorf("resolve project root: %w", err)
	}
	envFiles, tlsCertFile, tlsKeyFile, err := resolveProjectPrivatePaths(absoluteRoot)
	if err != nil {
		return "", err
	}
...
func resolveProjectPrivatePaths(projectRoot string) ([]string, string, string, error) {
	...
	if !isPrivateSource && !isNestedFilterestInstallation(projectRoot) && !homes.KeysHomeConfigured {
		return []string{

...
func TestResolveAppDBCompatibilityManifestPathSupportsConfiguredComposition(t *testing.T) {
	projectRoot := t.TempDir()
	configuredPath := "configured_source/server_tools/versioning/app_db_compatibility.jsonl"
	want := filepath.Join(projectRoot, configuredPath)
	if got := resolveAppDBCompatibilityManifestPath(projectRoot, configuredPath); got != want {
		t.Fatalf("resolveAppDBCompatibilityManifestPath() = %q, want %q", got, want)
	}
}

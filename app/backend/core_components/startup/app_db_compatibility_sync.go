...
func SyncAppDBCompatibilityMirror(
	db *sql.DB,
	projectRoot string,
	...
	manifestPath := ""
	if len(configuredManifestPath) > 0 {
		manifestPath = configuredManifestPath[0]
	}
	rows, err := loadAppDBCompatibilityManifest(
		resolveAppDBCompatibilityManifestPath(projectRoot, manifestPath),
	)
	if err != nil {
		return 0, err
	}
...
func resolveAppDBCompatibilityManifestPath(projectRoot, configuredPath string) string {

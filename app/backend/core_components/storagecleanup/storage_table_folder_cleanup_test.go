...
func withWorkingDirectory(t *testing.T) string {
	...
	paths, err := runtimepaths.Resolve(tempDir, tempDir, false)
	if err != nil {
		t.Fatalf("runtimepaths.Resolve: %v", err)
	}
	if err := runtimepaths.Configure(paths); err != nil {
		t.Fatalf("runtimepaths.Configure: %v", err)
	}
	t.Cleanup(func() {
		_ = os.Chdir(oldWD)
		legacyPaths, resolveErr := runtimepaths.Resolve(oldWD, oldWD, false)
		if resolveErr != nil {
			t.Errorf("runtimepaths.Resolve cleanup: %v", resolveErr)
			return
		}
		if configureErr := runtimepaths.Configure(legacyPaths); configureErr != nil {
			t.Errorf("runtimepaths.Configure cleanup: %v", configureErr)
		}
	})
	return tempDir
}
...
func TestArchiveTableStorageFolderUsesConfiguredRootsOutsideWorkingDirectory(t *testing.T) {
	withWorkingDirectory(t)
	...
	sourceFile := filepath.Join(paths.StorageRoot, "2868", "1", "original", "new.png")
	...
	if _, err := os.Stat(filepath.Join(paths.StorageRoot, "2868")); !os.IsNotExist(err) {
		t.Fatalf("configured storage root still exists after archive: %v", err)
	}
	archivedFile := filepath.Join(
		paths.StorageDeletedRoot,
		"2868",
		"1",
		"original",
		"new.png",
	...
	if _, err := os.Stat(archivedFile); err != nil {
		t.Fatalf("configured archived file missing: %v", err)
	}
}

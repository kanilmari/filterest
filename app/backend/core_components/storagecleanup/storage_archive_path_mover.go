...
func MovePathToDeletedStorage(sourcePath, destinationPath string) error {
	...
	sourceRoot, err := resolveStorageRootDirectory(roots.StorageRoot, false)
	if err != nil {
		return err
	}
	destinationRoot, err := resolveStorageRootDirectory(roots.StorageDeletedRoot, true)
	if err != nil {
		return err
	}

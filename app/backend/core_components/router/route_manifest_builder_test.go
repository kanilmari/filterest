...
func TestBuildDefaultRouteManifestCoversScenarioMatrix(t *testing.T) {
	...
	if personalSortDefault.PathPattern != "/api/dataset-sort-default/personal" {
		t.Fatalf("personal dataset sort default path = %q", personalSortDefault.PathPattern)
	}
	if mustFindScenarioProfile(t, personalSortDefault, "production").ProfileName != "login_only" {
		t.Fatalf("personal dataset sort default must use login-only profile")
	}

	adminSortDefault := mustFindManifestRoute(t, manifest, "system_table_tools.SaveDatasetSortDefaultHandler")
	assertRouteMethods(t, adminSortDefault, []string{"POST"}, router.RouteMethodSourceExplicitStableContract)
	if adminSortDefault.PathPattern != "/api/admin/dataset-sort-default" {
		t.Fatalf("administrator dataset sort default path = %q", adminSortDefault.PathPattern)
	}
	if mustFindScenarioProfile(t, adminSortDefault, "production").ProfileName != "admin" {
		t.Fatalf("administrator dataset sort default must retain admin profile")
	}

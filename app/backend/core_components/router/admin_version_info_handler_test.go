...
func TestAdminVersionInfoHandlerReturnsReadinessVersions(t *testing.T) {
	...
	if response.LatestReleaseURL == "" || response.UpdateCheckedAt == "" || response.RefreshAllowedAt == "" {
		t.Fatalf("release update metadata = %#v, want URL and checked time", response)
	}
	if !response.UpstreamCheckPerformed {
		t.Fatalf("release update metadata = %#v, want upstream check evidence", response)
	}
...
func TestAdminVersionInfoHandlerPostChecksAgain(t *testing.T) {
	restoreProbe := replaceSystemReadinessProbe(func() systemReadyResponse {
		return systemReadyResponse{AppVersion: "8.27.99"}
	})
	defer restoreProbe()
	originalCheck := adminLatestStableReleaseCheck
	adminLatestStableReleaseCheck = func(context.Context, string) releaseupdates.Status {
		t.Fatal("POST must use only the forced release check")
		return releaseupdates.Status{}
	}
	defer func() { adminLatestStableReleaseCheck = originalCheck }()
	originalCheckNow := adminLatestStableReleaseCheckNow
	adminLatestStableReleaseCheckNow = func(_ context.Context, currentVersion string) releaseupdates.Status {
		if currentVersion != "8.27.99" {
			t.Fatalf("current version = %q, want 8.27.99", currentVersion)
		}
		return releaseupdates.Status{
			LatestStableVersion:    "8.28.1",
			UpdateStatus:           releaseupdates.UpdateStatusAvailable,
			UpdateAvailable:        true,
			CheckedAt:              "2026-08-25T18:00:00Z",
			RefreshAllowedAt:       "2026-08-25T18:00:30Z",
			UpstreamCheckPerformed: true,
		}
	}
	defer func() { adminLatestStableReleaseCheckNow = originalCheckNow }()

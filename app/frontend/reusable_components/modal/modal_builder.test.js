...
describe("modal_builder accessibility", () => {
    ...
    test("keeps a transitioning modal mounted until its close animation ends", () => {
        const cleanup = vi.fn();
        const prepareTransition = vi.fn();
        const { modal_overlay } = createModal({
            titlePlainText: "Animated dialog",
            contentElements: [document.createElement("div")],
            cleanupCallback: cleanup,
            closeTransition: {
                className: "dialog-closing",
                durationMs: 300,
                beforeStart: prepareTransition,
            },
        });
        showModal();
        ...
        hideModal();
        expect(prepareTransition).toHaveBeenCalledOnce();
        expect(modal_overlay.classList.contains("dialog-closing")).toBe(true);
        expect(modal_overlay.style.display).toBe("flex");
        expect(modal_overlay.getAttribute("aria-busy")).toBe("true");
        ...
        expect(modal_overlay.classList.contains("dialog-closing")).toBe(false);
        expect(modal_overlay.style.display).toBe("none");
        expect(modal_overlay.getAttribute("aria-hidden")).toBe("true");
    });

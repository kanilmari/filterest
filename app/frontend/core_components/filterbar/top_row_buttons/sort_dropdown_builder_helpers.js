...
export const NEWEST_SORT_COLUMN = "__newest";
export const NEWEST_SORT_DIRECTION = "DESC";
export const NEWEST_SORT_VALUE = `${NEWEST_SORT_COLUMN}:${NEWEST_SORT_DIRECTION}`;
 ...
 */
export function buildSortOptions(sortableColumns, availableColumns = []) {
    ...
    const options = [
        ...
        { value: NEWEST_SORT_VALUE, label: "Newest", langKey: "sort_newest" },
        { value: `${NEWEST_SORT_COLUMN}:ASC`, label: "Oldest", langKey: "sort_oldest" },
    ];

    if (availableColumnSet.has("updated")) {
        options.push(
...
/**
 * Keep a route/state selection representable by this dataset's dropdown.
 * Unknown legacy values fall back to the semantic newest/oldest contract,
 * which is also what the server applies for unavailable columns.
 */
export function normalizeSortSelection(value, availableValues) {
    const normalizedValue = String(value || "").trim();
    if (!normalizedValue || availableValues.has(normalizedValue)) {
        return normalizedValue;
    }

    const separator = normalizedValue.lastIndexOf(":");
    const direction = separator >= 0
        ? normalizedValue.slice(separator + 1).trim().toUpperCase()
        : "";
    if (direction === "ASC" || direction === "DESC") {
        return `${NEWEST_SORT_COLUMN}:${direction}`;
    }
    return NEWEST_SORT_VALUE;
}

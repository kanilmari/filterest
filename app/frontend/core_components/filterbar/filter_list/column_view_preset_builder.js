...
export function buildColumnViewPresetSelector(tableName, columns = [], requestedViewKey = "table") {
    ...
    sourceStatus.classList.add("column-preset-source-status", "fw-text-muted", "fw-text-sm");
    sourceStatus.dataset.testid = "field-set-effective-source";
    content.appendChild(sourceStatus);

    const assignmentDiagnostic = document.createElement("p");
    assignmentDiagnostic.classList.add("column-preset-assignment-diagnostic", "fw-text-muted", "fw-text-sm");
    assignmentDiagnostic.dataset.testid = "field-set-assignment-diagnostic";
    assignmentDiagnostic.setAttribute("role", "status");
    assignmentDiagnostic.hidden = true;
    content.appendChild(assignmentDiagnostic);
    ...
    function render() {
        ...
        sourceStatus.dataset.effectiveScope = effectiveScope;
        sourceStatus.dataset.langKey = editingSiteDefault
            ? "field_set_editing_site_default"
            : `field_set_source_${effectiveScope}`;
        sourceStatus.textContent = editingSiteDefault
            ? t("field_set_editing_site_default", "Muokataan sivuston oletusta (vain jaetut kenttäjoukot)")
            : effectiveFieldSetSourceLabel(effectiveScope);

        const missingAssignments = missingFieldSetAssignmentScopes({
            fieldSets,
            activeFieldSetID,
            personalFieldSetID,
            siteDefaultFieldSetID,
        });
        assignmentDiagnostic.hidden = !serverCollectionsAvailable || missingAssignments.length === 0;
        assignmentDiagnostic.dataset.missingScopes = missingAssignments.join(",");
        if (missingAssignments.length > 0) {
            assignmentDiagnostic.dataset.langKey = "field_set_assignment_unavailable";
        } else {
            delete assignmentDiagnostic.dataset.langKey;
        }
        assignmentDiagnostic.textContent = missingAssignments.length > 0
            ? t(
                "field_set_assignment_unavailable",
                "Tallennettu kenttäjoukkovalinta ei ole käytettävissä. Palvelimen turvallinen kenttävalinta on edelleen käytössä.",
            )
            : "";
        select.innerHTML = "";
        ...
        for (const fieldSet of availableSets()) {
            ...
            option.dataset.ownerScope = fieldSet.scope;
            option.textContent = fieldSet.scope === "shared"
                ? `${fieldSet.name} (${t("shared", "jaettu")})`
                : `${fieldSet.name} (${t("field_set_owner_personal", "henkilökohtainen")})`;
        ...
        const resetUsesSiteDefault = Boolean(siteDefaultFieldSetID);
        resetButton.dataset.langKey = resetUsesSiteDefault
            ? "use_site_default"
            : "use_metadata_default";
        resetButton.textContent = resetUsesSiteDefault
            ? t("use_site_default", "Käytä sivuston oletusta")
            : t("use_metadata_default", "Käytä metadatan oletusta");
    }
    ...
    async function reload({ applyEffective = false } = {}) {
        ...
        effectiveScope = normalizeEffectiveFieldSetScope(response?.effective_scope);
    ...
    addListener(resetButton, "click", async () => {
        try {
            // This endpoint deletes only the signed-in user's assignment.
            // The site or metadata layer is inherited without being rewritten.
            await resetPersonalViewFieldSet({ dataset: tableName, view_key: viewKey });
            activeFieldSetID = null;
            await reload({ applyEffective: true });
            await refreshDatasetAfterFieldSetMutation();
            showSuccessToast(t("field_set_inheritance_restored", "Oma ohitus poistettiin"));
        } catch (error) {
...
export function effectiveFieldSetSourceLabel(scope) {
    ...
    default:
        return t("field_set_source_metadata", "Metadatan oletus on käytössä");
    }
}

export function missingFieldSetAssignmentScopes({
    fieldSets = [],
    activeFieldSetID = null,
    personalFieldSetID = null,
    siteDefaultFieldSetID = null,
} = {}) {
    const returnedIDs = new Set(fieldSets.map((fieldSet) => String(fieldSet?.id)));
    const missing = [];
    if (personalFieldSetID !== null && !returnedIDs.has(String(personalFieldSetID))) {
        missing.push("personal");
    }
    if (siteDefaultFieldSetID !== null && !returnedIDs.has(String(siteDefaultFieldSetID))) {
        missing.push("site");
    }
    if (
        activeFieldSetID !== null
        && !returnedIDs.has(String(activeFieldSetID))
        && String(activeFieldSetID) !== String(personalFieldSetID)
        && String(activeFieldSetID) !== String(siteDefaultFieldSetID)
    ) {
        missing.push("active");
    }
    return missing;

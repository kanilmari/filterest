...
describe('create_filter_bar inline hero mounting', () => {
    ...
    test('keeps inline hero count-free and wires reset search to the shared filter reset', async () => {
        ...
        const sortDropdown = inlineHero?.querySelector('select');
        const filtersButton = inlineHero?.querySelector(
            '[data-testid="temporary-filters-toggle-content-hero"]'
        );
        const sortRow = inlineHero?.querySelector('.filterbar-inline-hero-sort-row');
        const filterActions = inlineHero?.querySelector(
        ...
        expect(filtersButton?.dataset.langKey).toBe('filterbar_filter_results');
        expect(sortRow?.contains(sortDropdown)).toBe(true);
        expect(sortRow?.contains(filtersButton)).toBe(false);
        expect(filterActions?.children).toHaveLength(2);
        expect(filterActions?.children[0]).toBe(filtersButton);
        expect(filterActions?.children[1]).toBe(resetButton);
        expect(
            document.querySelector(
                '[data-testid="temporary-filters-toggle-filter-stack"]'
            )
        ).toBeNull();

        const sharedCenter = document.querySelector(
            '.dataset-shared-topbar__center'
        );
        expect(sharedCenter?.children).toHaveLength(1);
        expect(sharedCenter?.firstElementChild?.classList).toContain(
            'dataset-search-panel'
        );
        expect(sharedCenter?.querySelector('select')).toBeNull();
        expect(sharedCenter?.querySelector('[data-temporary-filters-toggle-for]'))
            .toBeNull();
        expect(sharedCenter?.querySelector('[data-testid^="btn-reset-search-"]'))
            .toBeNull();

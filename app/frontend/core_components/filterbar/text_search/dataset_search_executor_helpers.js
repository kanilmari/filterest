...
import {
    ...
    NEWEST_SORT_COLUMN,
} from "../top_row_buttons/sort_dropdown_builder_helpers.js";
...
function resolveNewestSortColumn(rows, columnTypes) {
    const candidates = ["created", "created_at", "applied_at", "id"];
    return candidates.find((column) =>
        Object.prototype.hasOwnProperty.call(columnTypes || {}, column)
        || rows.some((row) => Object.prototype.hasOwnProperty.call(row || {}, column))
    ) || null;
}
...
export function sortRows(rows, sortColumn, sortOrder, columnTypes = {}) {
    ...
    if (column === NEWEST_SORT_COLUMN) {
        const newestColumn = resolveNewestSortColumn(safeRows, columnTypes);
        return newestColumn
            ? sortRows(safeRows, newestColumn, direction, columnTypes)
            : safeRows;
    }

...
// Verifies admin-only views stay lazy so light shells do not download them.
// Bridges the custom-view registry source with the login/catalog module graph.
// Exists so /login cannot statically import queen_chat_view.js.
...
describe("custom_view_reader module graph", () => {
    test("does not statically import queen_chat_view.js", () => {
        const source = fs.readFileSync(sourcePath, "utf8");
        expect(source).not.toMatch(
            /import\s*\{[^}]*generate_queen_chat_view[^}]*\}\s*from\s*['"][^'"]*queen_chat_view\.js['"]/
        );
        expect(source).toContain("await import('../../admin_tools/queen_chat_view.js')");
    });

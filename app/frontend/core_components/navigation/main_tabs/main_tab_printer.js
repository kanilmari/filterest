...
import { fetchProjectTabs, acceptMainDatasetTabs, AUTH_ACTION_TAB_IDS } from "./main_dataset_tabs.js";
...
export async function initTabs({ dataAlreadyLoaded = false, preloadedContentTablesResponse = null } = {}) {
    ...
    if (renderGeneration !== _tabRenderGeneration || !acceptMainDatasetTabs(tabResult, isLoggedIn)) {
        return;
    }

    const allTabs = tabResult.tabs;
...
export async function openNavTab(tableName, options = {}) {
    ...
    const { skipNavigation = false, skipUrlUpdate = false, forceReload = false, replacementParams } = options;
    ...
    if (!skipNavigation) {
        useStorageParams();
        const result = await handle_all_navigation(tableName, custom_views, {
            skipUrlUpdate, forceReload, ...(replacementParams === undefined ? {} : { replacementParams }),
        });
        useUrlParams();

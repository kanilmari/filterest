...
vi.mock("../endpoints/stable_endpoint_router.js", () => ({
    checkAdminVersionInfoAgain: checkAdminVersionInfoAgainMock,
...
describe("admin version info indicator", () => {
    beforeEach(() => {
        ...
        checkAdminVersionInfoAgainMock.mockReset();
    ...
    test("does not render without the protected route permission", async () => {
        ...
        expect(checkAdminVersionInfoAgainMock).not.toHaveBeenCalled();
    });
    ...
    test("hydrates a shared info icon and toggles the version panel by click", async () => {
        ...
        fetchAdminVersionInfoMock.mockResolvedValue({
            ...
            update_checked_at: "2026-08-25T18:00:00Z",
            refresh_allowed_at: "2026-08-25T18:00:30Z",
    ...
    test("checks again on demand and replaces the open version snapshot", async () => {
        hasRoutePermissionMock.mockReturnValue(true);
        fetchAdminVersionInfoMock.mockResolvedValue({
            product_name: "Filterest",
            app_version: "8.40.8",
            latest_stable_version: "8.40.8",
            update_status: "current",
            update_available: false,
            update_checked_at: "2026-08-25T17:00:00Z",
            db_version: "9.6.2",
            required_db_version: "9.6.2",
            db_compatible: true,
            runtime_mode: "docker",
        });
        checkAdminVersionInfoAgainMock.mockResolvedValue({
            product_name: "Filterest",
            app_version: "8.40.8",
            latest_stable_version: "8.41.0",
            update_status: "available",
            update_available: true,
            update_checked_at: "2026-08-25T18:00:00Z",
            upstream_check_performed: true,
    ...
    test("refreshes the open panel every five minutes and stops polling when closed", async () => {
        vi.useFakeTimers();
        hasRoutePermissionMock.mockReturnValue(true);
        fetchAdminVersionInfoMock
            ...
            .mockResolvedValueOnce({
                product_name: "Filterest",
                app_version: "8.41.0",
                latest_stable_version: "8.42.0",
                update_status: "available",
                update_available: true,
                update_checked_at: "2026-08-25T18:05:00Z",
                db_version: "9.6.4",
                required_db_version: "9.6.4",
                db_compatible: true,
                runtime_mode: "docker",
        ...
        const {
            ...
            buildAdminVersionInfoIndicator,
        } = await import("./admin_version_info_indicator.js");
        const shell = buildAdminVersionInfoIndicator();
        document.body.appendChild(shell);
        const indicator = shell.querySelector('[data-testid="filterbar-admin-version-info"]');
        const panel = shell.querySelector('[data-testid="filterbar-admin-version-info-panel"]');
        ...
        expect(panel.querySelector('[data-version-info-value="latest-stable"]')?.textContent)
            .toContain("8.41.0");
        expect(fetchAdminVersionInfoMock).toHaveBeenCalledTimes(1);

        await vi.advanceTimersByTimeAsync(ADMIN_VERSION_INFO_OPEN_REFRESH_INTERVAL_MS - 1);
        expect(fetchAdminVersionInfoMock).toHaveBeenCalledTimes(1);
        ...
        indicator.click();
        expect(panel.hidden).toBe(true);
        await vi.advanceTimersByTimeAsync(ADMIN_VERSION_INFO_OPEN_REFRESH_INTERVAL_MS * 2);
        expect(fetchAdminVersionInfoMock).toHaveBeenCalledTimes(2);

        shell.destroy();
    });

    test("keeps one refresh timer across reopen and cancels it on removal", async () => {
        vi.useFakeTimers();
        hasRoutePermissionMock.mockReturnValue(true);
        fetchAdminVersionInfoMock.mockResolvedValue({
            product_name: "Filterest",
        ...
        indicator.click();
        await vi.waitFor(() => expect(fetchAdminVersionInfoMock).toHaveBeenCalledTimes(1));
        expect(vi.getTimerCount()).toBe(1);
        indicator.click();
        expect(vi.getTimerCount()).toBe(0);

        indicator.click();
        await vi.waitFor(() => expect(fetchAdminVersionInfoMock).toHaveBeenCalledTimes(2));
        expect(vi.getTimerCount()).toBe(1);
        indicator.click();
        indicator.click();
        await vi.waitFor(() => expect(fetchAdminVersionInfoMock).toHaveBeenCalledTimes(3));
        expect(vi.getTimerCount()).toBe(1);

        await vi.advanceTimersByTimeAsync(ADMIN_VERSION_INFO_OPEN_REFRESH_INTERVAL_MS);
        expect(fetchAdminVersionInfoMock).toHaveBeenCalledTimes(4);
        shell.remove();
    ...
    test.each([
        ...
        await vi.waitFor(() => {
            expect(panel.querySelector(
                '[data-testid="filterbar-admin-version-check-again"]'
            )?.textContent).toBe(expectedLabel);
        });

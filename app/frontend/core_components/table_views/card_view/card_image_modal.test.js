...
describe("card image modal", () => {
    ...
    test("tears down image listeners, timers, and classes before an ordinary modal", async () => {
        ...
        imageModal.close();
        expect(overlay.classList.contains("image-first-view-closing")).toBe(true);
    ...
    test("hands off between records without closing the image-first modal", async () => {
        const {
            openImageModalContent,
            transitionImageFirstModalContent,
        } = await import("./card_image_modal.js");
        const outgoingView = document.createElement("div");
        outgoingView.className = "image_first_view";
        const outgoingStage = document.createElement("div");
        outgoingStage.className = "row_article_image_first_stage";
        outgoingStage.style.setProperty(
            "--row-article-image-first-backdrop",
            "url(old.png)",
        );
        outgoingView.appendChild(outgoingStage);

        openImageModalContent({
            contentElement: outgoingView,
            classNames: ["image_first_view_modal"],
            overlayClassNames: ["image_first_view_overlay"],
        });

        const incomingView = document.createElement("div");
        incomingView.className = "image_first_view";
        const incomingStage = document.createElement("div");
        incomingStage.className = "row_article_image_first_stage";
        incomingStage.style.setProperty(
            "--row-article-image-first-backdrop",
            "url(new.png)",
        );
        incomingView.appendChild(incomingStage);
        const navigation = document.createElement("nav");
        const navigationButton = document.createElement("button");
        navigation.appendChild(navigationButton);

        const result = transitionImageFirstModalContent({
            contentElement: incomingView,
            topControlElements: [navigation],
        });
        const body = result.modal.querySelector(":scope > .modal_body");

        expect(result.modalOverlay.classList).toContain(
            "image-first-record-transitioning",
        );
        expect(Array.from(body.children)).toEqual([outgoingView, incomingView]);
        expect(outgoingView.classList).toContain("image_first_view--outgoing");
        expect(incomingView.classList).toContain("image_first_view--incoming");
        expect(navigationButton.disabled).toBe(false);
        expect(navigation.getAttribute("aria-busy")).toBe("true");
        expect(body.getAttribute("aria-busy")).toBe("true");

        vi.advanceTimersByTime(150);
        expect(outgoingStage.style.getPropertyValue(
            "--row-article-image-first-backdrop",
        )).toBe("url(new.png)");

        vi.advanceTimersByTime(150);
        expect(Array.from(body.children)).toEqual([incomingView]);
        expect(incomingView.classList).not.toContain("image_first_view--incoming");
        expect(incomingView.classList).toContain("image_first_view--settled");
        expect(result.modalOverlay.classList).not.toContain(
            "image-first-record-transitioning",
        );
        expect(navigationButton.disabled).toBe(false);
        expect(navigation.hasAttribute("aria-busy")).toBe(false);

...
// Coordinates article-view UI state, scroll handling, and highlighted-card behavior.
...
/** Scroll position saved before opening an article, restored on close. */
let _savedScrollY = 0;
export function saveScrollBeforeRowArticle() { _savedScrollY = window.scrollY; }
export function restoreScrollAfterRowArticle() { window.scrollTo(0, _savedScrollY); }
...
export function createRowArticleKeyValueElement(
    ...
    count_this_function("createTwoLineKeyValueElement"); // 🔢 stored counter key, kept stable
    ...
    if (hasLangKey) {
        ...
        const resolved = resolveRowArticleLocalizedValue(value, isMultilingual);
...
export function createRowArticleNavigableElement({
    ...
    count_this_function("createNavigableTwoLineElement"); // 🔢 stored counter key, kept stable
...
export function closeRowArticle(
    wrapper,
    card_container,
    big_card_div,
    selectedCard,
    ...
    // A flag tells the popstate handler to skip re-navigation since closeRowArticle
    // already cleaned up the DOM.
    // If the current state was NOT pushed by openRowArticleView (e.g. direct URL),
    ...
    if (!skipHistoryBack && table_name) {
        ...
        if (isDatasetRowPath(currentPath, DATASET_PREFIX, table_name)) {
            if (history.state?.bigCard === true && history.state?.articleReturnAvailable !== false) {
    ...
    if (restoreScroll) restoreScrollAfterRowArticle();
    ...
    if (table_name) {
        setUnifiedTableState(table_name, {
            [wrapper.dataset.viewKey === "article_view" ? "articleView" : "cardView"]: { collapsed: false, expandedId: null },
        });
    }
}

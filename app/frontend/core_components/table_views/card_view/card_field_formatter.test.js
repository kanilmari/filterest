...
describe('card_field_formatter multilingual editing', () => {
    ...
    test('keeps the primary key read-only and never collects it as an article update', () => {
        setColumnDetails([
            { table_name: 'dev_agent_tasks', column_name: 'id', editable_in_ui: true, data_type: 'integer' },
            { table_name: 'dev_agent_tasks', column_name: 'title', editable_in_ui: true, data_type: 'text' },
        ]);
        const container = document.createElement('div');
    ...
    test('keeps an unchanged nullable numeric field out of article updates', () => {
        setColumnDetails([
            { table_name: 'dev_agent_tasks', column_name: 'assigned_to', editable_in_ui: true, data_type: 'integer' },
        ]);
        const container = document.createElement('div');
    ...
    test('unwraps selection values and rejects unknown objects before updateRow', async () => {
        endpointRouterMock.mockResolvedValue({ ok: true });

        let caughtError;
        try {
            await sendCardUpdates('dev_agent_tasks', 869, {
                id: '',
                priority: { value: 'normal', label: 'Normal' },
                assigned_to: { unexpected: 'shape' },
                title: 'Saved title',
            });
        } catch (error) {
            caughtError = error;
        ...
        expect(endpointRouterMock.mock.calls[0][1].body_data).toEqual({
            id: 869, column: 'priority', value: 'normal',
        });
        expect(endpointRouterMock.mock.calls[1][1].body_data).toEqual({
            id: 869, column: 'title', value: 'Saved title',
        });
        expect(caughtError).toMatchObject({
            name: 'CardUpdateError',
            successfulFields: ['priority', 'title'],
        });
        expect(caughtError.failedFields).toEqual([
            expect.objectContaining({ column: 'assigned_to', attempted: false }),
        ]);
    });

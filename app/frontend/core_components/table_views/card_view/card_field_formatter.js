...
export function createKeyValueElement(
    ...
    valueDiv.setAttribute("data-raw-value", String(raw_value ?? ''));
...
export function collectCardUpdates(container) {
    ...
    const updates = disableEditing(snapshot);
    delete updates.id;
    return updates;
}
...
export async function sendCardUpdates(table_name, rowId, updatedData) {
    ...
    const entries = Object.entries(updatedData).filter(([column]) => column !== 'id');
    const successfulFields = [];
    const failedFields = [];

    for (let index = 0; index < entries.length; index += 1) {
        const [column, value] = entries[index];
        const transportValue = normalizeCardUpdateTransportValue(value);
        if (!transportValue.valid) {
            failedFields.push({
                column,
                error: new TypeError('Article field value must be a scalar.'),
                attempted: false,
            });
            continue;
        }
        const normalizedValue = isTicketStatusField(table_name, column)
            ? normalizeTicketStatusForDb(transportValue.value)
...
// normalizeCardUpdateTransportValue unwraps selection-shaped UI values without
// serializing arbitrary objects into scalar database columns. Unsupported values
// stay in the editor draft and are reported before any update request is sent.
export function normalizeCardUpdateTransportValue(value) {
    ...
    for (const key of ['value', 'id', 'slug', 'key']) {
...
export function enableEditing(container, table_name) {
    ...
    textFields.forEach((fieldElem) => {
        const columnName = fieldElem.getAttribute('data-column');
        if (columnName === 'id') {
            if (IS_DEV_MODE) console.log(`[${table_name}] primary key id stays read-only in article editing.`);
            return;
        }

...
describe('config_fetcher', () => {
  ...
  test('keeps every dataset newest-first in the checked-in config', () => {
    expect(checkedInConfig.default_dataset_sort).toEqual({
      column: '__newest',
      direction: 'DESC',
    });
    expect(checkedInConfig.default_dataset_sorts).toBeUndefined();
  });
});

...
import { isFilterbarAIChatDevEnvironment } from "./table_chat_mode_resolver.js";
...
export function createCodingAgentControl(dataset) {
 if (!hasRoutePermission("/api/app/ai-chat/codex-query")) return null;
 ...
 let destroyed = false;
 const legacy = isFilterbarAIChatDevEnvironment();
 ...
 select.value = legacy && preferred === "codex_dev" ? "codex_dev" : "api_tools";
 function render() {
  if (destroyed) return;
  ...
  const permitted = capability ? capability.feature_enabled === true : legacy;
  row.hidden = capability ? !permitted : !legacy;
  agent.disabled = capability ? !(permitted && capability.runner_ready === true) : !legacy;
 ...
 render();
 endpoint_router("aiChatCodexQuery", { method: "GET", url_params: new URLSearchParams({ dataset }).toString(), suppressErrorToast: true }).then(capability => {
  if (destroyed || typeof capability?.feature_enabled !== "boolean") return;
  ...
  if (!destroyed) { control.capability = { feature_enabled: legacy, runner_ready: legacy }; render(); }
 });
 return control;
}

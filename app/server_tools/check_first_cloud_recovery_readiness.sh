...
PROJECT_ROOT="${FIRST_CLOUD_RECOVERY_SOURCE_ROOT:-$(cd "${SCRIPT_DIR}/.." && pwd)}"
APP_VERSION_MARKER="${FIRST_CLOUD_RECOVERY_APP_VERSION_MARKER:-VERSION_APP}"
APP_BINARY_NAME="${FIRST_CLOUD_RECOVERY_APP_BINARY_NAME:-filterest}"
if [[ ! "$APP_VERSION_MARKER" =~ ^VERSION_[A-Z][A-Z0-9_]*$ ]]; then
    printf 'error: invalid app-version marker: %s\n' "$APP_VERSION_MARKER" >&2
    exit 2
fi
if [[ ! "$APP_BINARY_NAME" =~ ^[a-z][a-z0-9_-]*$ ]]; then
    printf 'error: invalid app binary name: %s\n' "$APP_BINARY_NAME" >&2
    exit 2
fi
...
EXPECTED_APP_VERSION="$(tr -d '[:space:]' <"$PROJECT_ROOT/$APP_VERSION_MARKER" 2>/dev/null || true)"
...
DEPLOYMENT_RUNBOOK_ROOT="${FIRST_CLOUD_RECOVERY_DEPLOYMENT_RUNBOOK_ROOT:-}"
COMPATIBILITY_MANIFEST="${FIRST_CLOUD_RECOVERY_COMPATIBILITY_MANIFEST:-server_tools/versioning/app_db_compatibility.jsonl}"
...
Usage:
  ...
  ${SCRIPT_NAME} --ssh-target user@example.com --ssh-key ~/.ssh/filterest_key --release-root /opt/filterest --backup-dir /srv/filterest-cloud/filterest.com/backups --base-url https://filterest.com
...
Options:
  ...
  --storage-deleted-root <path>
                              ...
                              inspect.
  --base-url <url>            Live Filterest base URL to inspect through
                              /system/ready and /system/instance-status.
                              May be repeated.
  --remote-base-url <url>     Live Filterest base URL to inspect over
  ...
  --require-current-version-match
                              Fail if the current release app-version marker or
...
run_manifest_validation() {
    local validator="$PROJECT_ROOT/server_tools/scripts/validate_app_db_compatibility.py"
    local manifest="$PROJECT_ROOT/server_tools/versioning/app_db_compatibility.jsonl"
    local output_file

    if [[ ! -r "$validator" || ! -r "$manifest" ]]; then
        path_missing_result \
            "$REQUIRE_REPO_RUNBOOKS" \
            "app/DB compatibility validation inputs are missing from this source tree"
        return
    fi
    if ! command -v python3 >/dev/null 2>&1; then
        path_missing_result "$REQUIRE_REPO_RUNBOOKS" "python3 is required to validate app/DB compatibility"
        return
    fi
    output_file="$(mktemp)"
...
print_local_restore_rehearsal_inputs() {
    ...
    if [[ -n "$release_root" ]]; then
        ...
        if [[ -L "$current_link" ]]; then
            ...
            if [[ -n "$current_target" && -r "$current_target/$APP_VERSION_MARKER" && -r "$current_target/VERSION_DB" ]]; then
                current_app_version="$(tr -d '[:space:]' <"$current_target/$APP_VERSION_MARKER" 2>/dev/null || true)"
...
print_remote_restore_rehearsal_inputs() {
    ...
    if ssh "${SSH_ARGS[@]}" "$SSH_TARGET" "bash -s" -- \
        "$release_root" "$backup_dir" "$EXPECTED_APP_VERSION" "$EXPECTED_DB_VERSION" \
        "$APP_VERSION_MARKER" >"$output_file" 2>"$error_file" <<'REMOTE'
release_root="$1"
backup_dir="$2"
expected_app_version="$3"
expected_db_version="$4"
app_version_marker="$5"
...
if [[ -n "$release_root" ]]; then
    ...
    if path_is_link "$current_link"; then
        ...
        if [[ -n "$current_target" ]]; then
            current_app_version="$(read_trimmed_file "$current_target/$app_version_marker" || true)"
...
check_repo_recovery_inputs() {
    ...
    if [[ -n "$DEPLOYMENT_RUNBOOK_ROOT" ]]; then
        check_repo_file "$DEPLOYMENT_RUNBOOK_ROOT/server_tools/deploy_to_production.sh" "deployment-owner native deploy script" "$REQUIRE_REPO_RUNBOOKS"
        check_repo_file "$DEPLOYMENT_RUNBOOK_ROOT/server_tools/daily_backup.sh" "deployment-owner native daily backup script" "$REQUIRE_REPO_RUNBOOKS"
    else
        info "deployment-owner runbooks are not part of this standalone Filterest source"
    fi
    check_repo_file "server_tools/check_first_cloud_runtime_safety.sh" "first-cloud runtime safety helper" "$REQUIRE_REPO_RUNBOOKS"
    check_repo_file "$COMPATIBILITY_MANIFEST" "app/DB compatibility manifest" "$REQUIRE_REPO_RUNBOOKS"
...
check_release_root() {
    ...
    if [[ -r "$current_target/$APP_VERSION_MARKER" && -r "$current_target/VERSION_DB" ]]; then
        local current_app_version
        local current_db_version

        current_app_version="$(tr -d '[:space:]' <"$current_target/$APP_VERSION_MARKER" 2>/dev/null || true)"
        current_db_version="$(tr -d '[:space:]' <"$current_target/VERSION_DB" 2>/dev/null || true)"
        pass_check "current release carries ${APP_VERSION_MARKER}=${current_app_version:-<empty>} and VERSION_DB=${current_db_version:-<empty>}"
        record_version_match_result "$APP_VERSION_MARKER" "$current_app_version" "$EXPECTED_APP_VERSION"
        record_version_match_result "VERSION_DB" "$current_db_version" "$EXPECTED_DB_VERSION"
    else
        fail_check "current release is missing ${APP_VERSION_MARKER} or VERSION_DB"
    fi
    ...
    if [[ -x "$current_target/$APP_BINARY_NAME" ]]; then
        pass_check "current release has executable ${APP_BINARY_NAME} binary"
    else
        fail_check "current release is missing executable ${APP_BINARY_NAME} binary"
    fi
...
check_remote_release_root() {
    ...
    if ssh "${SSH_ARGS[@]}" "$SSH_TARGET" "bash -s" -- \
        ...
        "$APP_VERSION_MARKER" "$APP_BINARY_NAME" "$EXPECTED_APP_VERSION" "$EXPECTED_DB_VERSION" \
        "$REQUIRE_CURRENT_VERSION_MATCH" >"$output_file" 2>"$error_file" <<'REMOTE'
root="$1"
require_release_root="$2"
require_rollback_target="$3"
app_version_marker="$4"
app_binary_name="$5"
expected_app_version="$6"
expected_db_version="$7"
require_current_version_match="$8"
...
if path_is_readable "$current_target/$app_version_marker" && path_is_readable "$current_target/VERSION_DB"; then
    current_app_version="$(read_protected_file "$current_target/$app_version_marker" 2>/dev/null || true)"
    current_db_version="$(read_protected_file "$current_target/VERSION_DB" 2>/dev/null || true)"
    pass "current release carries ${app_version_marker}=${current_app_version:-<empty>} and VERSION_DB=${current_db_version:-<empty>}"
    version_match_result "$app_version_marker" "$current_app_version" "$expected_app_version"
    version_match_result "VERSION_DB" "$current_db_version" "$expected_db_version"
else
    fail "current release is missing ${app_version_marker} or VERSION_DB"
fi
...
if path_is_executable "$current_target/$app_binary_name"; then
    pass "current release has executable ${app_binary_name} binary"
else
    fail "current release is missing executable ${app_binary_name} binary"
fi
...
info "Filterest first-cloud recovery readiness check is non-mutating."

...
-- Creates a missing view without replacing existing definitions, site copy, or user access.
-- VERSION_DB: 9.7.11

-- Some older bootstraps recorded the creating migration but omitted its SQL view.
-- Restore only that absent read model; keep any existing site-owned definition.
DO $repair_missing_view$
BEGIN
    IF to_regclass('public.system_column_supported_views') IS NULL THEN
        EXECUTE $view_definition$
CREATE VIEW public.system_column_supported_views AS
SELECT

...
-- Adds one filterable read model across every registered column and view type.
-- Bridges column metadata, dataset identity, and the stable view registry without
-- conflating global capability with personal, group, or site field assignments.
-- VERSION_DB: 9.7.3

DO $$
BEGIN
    IF to_regclass('public.system_column_details') IS NULL
       OR to_regclass('public.system_db_tables') IS NULL
       OR to_regclass('public.system_table_views') IS NULL THEN
        RAISE EXCEPTION 'column support matrix prerequisites are missing';
    END IF;
END $$;
...
SELECT '9.7.3', 'Added the filterable column-to-supported-views metadata matrix.'
WHERE NOT EXISTS (
    SELECT 1 FROM public.system_db_version WHERE version = '9.7.3'
);

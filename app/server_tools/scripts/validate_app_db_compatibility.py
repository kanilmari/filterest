...
PUBLIC_BOOTSTRAP_FILES = (
    "server_tools/public_bootstrap/schema.sql",
    "server_tools/public_bootstrap/seed_data.sql",
)
...
def validate_repo_artifact_path(
    *,
    ...
    resolved_artifact_path = (artifact_root / artifact_path).resolve()

    try:
        tracked_path = resolved_artifact_path.relative_to(repo_root_resolved)
    except ValueError:
        errors.append(
...
def validate_public_bootstrap(
    repo_root: Path,
    current_app_version: str,
    current_db_version: str,
) -> int:
    """Validate Gitless canonical Filterest through its self-contained bootstrap."""

    manifest_path = repo_root / "server_tools/public_bootstrap/manifest.json"
    ...
    try:
        ...
        print(f"❌ Public bootstrap manifest is invalid JSON: {exc}")
        return 1

    if manifest.get("artifact") != "filterest-public-bootstrap":
        errors.append("public bootstrap artifact must be filterest-public-bootstrap")
    if manifest.get("format_version") != 2:
        errors.append("public bootstrap format_version must be 2")
    if manifest.get("app_version") != current_app_version:
        errors.append(
            "public bootstrap app_version "
            f"{manifest.get('app_version')!r} does not match VERSION_APP {current_app_version}"
        )
    if manifest.get("db_version") != current_db_version:
        errors.append(
            "public bootstrap db_version "
            f"{manifest.get('db_version')!r} does not match VERSION_DB {current_db_version}"
        )

    generated_files = manifest.get("generated_files")
    if not isinstance(generated_files, dict):
        errors.append("public bootstrap generated_files must be an object")
        generated_files = {}

    for relative_path in PUBLIC_BOOTSTRAP_FILES:
        artifact_path = repo_root / relative_path
        record = generated_files.get(relative_path)
        if not artifact_path.is_file():
            errors.append(f"public bootstrap artifact is missing: {relative_path}")
            continue
        if not isinstance(record, dict) or not isinstance(record.get("sha256"), str):
            errors.append(f"public bootstrap manifest lacks SHA-256 for {relative_path}")
            continue
        actual_sha256 = hashlib.sha256(artifact_path.read_bytes()).hexdigest()
        if record["sha256"] != actual_sha256:
            errors.append(
                f"public bootstrap SHA-256 mismatch for {relative_path}: "
                f"expected {record['sha256']}, got {actual_sha256}"
            )

    if errors:
        print("❌ Public bootstrap compatibility validation failed:")
        for error in errors:
            print(f" - {error}")
        return 1

    print(
        "✅ Public bootstrap compatibility OK: "
...
def resolve_repository_path(repo_root: Path, configured_path: Path) -> Path:
    """Resolve one configured path while keeping it inside the source root."""
...
def validate_manifest(
    ...
    try:
        manifest_path = resolve_repository_path(
            repo_root,
            configured_manifest
            or Path("server_tools/versioning/app_db_compatibility.jsonl"),
        )
    ...
    if not manifest_path.exists():
        if configured_manifest is not None:
            print(f"❌ Missing compatibility manifest: {manifest_path}")
            return 1
        return validate_public_bootstrap(
            repo_root,
            current_app_version,
            current_db_version,
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--repository-root",
        type=Path,
        help="validate another composed source root instead of canonical Filterest",
    )
    parser.add_argument(
        "--manifest-path",
        type=Path,
        help="use this repository-relative compatibility manifest instead of the public default",
    )
    parser.add_argument(
        "--artifact-root",
        type=Path,
        help="resolve manifest artifact paths below this repository-relative source root",
    )

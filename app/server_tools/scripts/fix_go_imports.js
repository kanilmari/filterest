...
const packageMap = {};
let projectModulePath = "";

// readProjectModulePath reads the authoritative import prefix from go.mod.
// Between a checkout's physical folder name and Go imports it keeps portable
// copies correct even when the directory is not literally named after the module.
function readProjectModulePath(projectRoot) {
    const goModPath = path.join(projectRoot, "go.mod");
    const goModContent = fs.readFileSync(goModPath, "utf8");
    const moduleMatch = /^\s*module\s+(\S+)\s*$/m.exec(goModContent);
    if (!moduleMatch) {
        throw new Error(`go.mod ei sisällä module-määritystä: ${goModPath}`);
    }
...
function buildPackageMap(baseDir, excludePatterns) {
    ...
    for (const goFile of allGoFiles) {
        ...
        global.importMap[`${projectModulePath}/${relPath}`] = dirPath;
    }
...
function processGoFile(filePath, fixImports, excludePatterns) {
    ...
    for (const importEntry of importsInFile) {
        ...
        else if (originalImportPath.startsWith(projectModulePath + "/")) {
            const adjustedImportPath = originalImportPath.substring(projectModulePath.length + 1);
...
function fixImportPathInFile({
    ...
    // Muodostetaan lopullinen import-polku, joka alkaa go.mod-moduulipolulla.
    const newFixedImportPath = `${projectModulePath}/${relativeToProjectRoot}`;
...
function paafunktio() {
    ...
    try {
        projectModulePath = readProjectModulePath(process.cwd());
    } catch (err) {
        console.error(`\x1b[31mvirhe: ${err.message}\x1b[0m`);
        process.exit(1);
    }

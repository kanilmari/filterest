...
function usage() {
    return `Usage:
  ${humanQaDisplayCommand} ai-test <ticket-id> --url <URL|route> [options]
...
Options:
  ...
  --auth-state <path>        Playwright storageState JSON. Default: per-target under FILTEREST_TEST_RUNTIME_ROOT/e2e/.auth/.
...
Examples:
  ${humanQaDisplayCommand} ai-test 837 --url /service_catalog --check "Firefox logo appears once" --assert-visible "img[alt*=Firefox]" --assert-no-text "duplicate logo"
  ${humanQaDisplayCommand} ai-test 0 --file docs/instructions_and_documentation/Human_QA_Handoff.md --assert-text "Human QA And AI Acceptance"`;
}

...
NODE_DEPENDENCY_ROOT="$SOURCE_ROOT"
if [[ "$SOURCE_ROOT" == "$INSTALLATION_ROOT/app" ]]; then
...
install_go_toolchain_if_needed() {
    ...
    required_version="$(awk '$1 == "go" {print $2; exit}' "$SOURCE_ROOT/go.mod")"
...
verified_legacy_database_identity() {
    ...
    required_version="$(tr -d '[:space:]' < "$DB_VERSION_FILE")"
...
ensure_admin_binary() {
    local target="$RUNTIME_ROOT/bin/filterest-server"
    local version_marker="$RUNTIME_ROOT/filterest-binary-version"
    local version=""
    local arch=""
    local asset=""
    local download_base=""
    ...
    if [[ -f "$APP_VERSION_FILE" ]]; then
        ...
        version="$(tr -d '[:space:]' < "$INSTALLATION_ROOT/VERSION_EASELECT")"
    fi
...
start_installed_filterest() {
    ...
    if [[ "$PROFILE" == "admin" ]]; then
        "$SOURCE_ROOT/server_tools/run_filterest_admin.sh"
    else

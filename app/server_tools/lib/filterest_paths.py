...
def is_nested_filterest_installation(project_root: Path) -> bool:
    """Return whether one install root contains the canonical immutable app."""

    app_root = project_root / "app"
    return (app_root / "go.mod").is_file() and (app_root / "VERSION_APP").is_file()
...
def resolve_filterest_homes(
    ...
    # Keep legacy flat public checkouts compatible while making every new nested
    # installation derive its operator-owned homes from the one Filterest root.
    ...
    config_paths = [root / CONFIG_FILE_NAME, root / LOCAL_CONFIG_FILE_NAME]
    if nested_public_install:
        config_paths.append(root / "config" / CONFIG_FILE_NAME)
    ...
    if nested_public_install:
        ...
        for name, home in resolved_homes.items():
            try:
                home.relative_to(immutable_app)
            except ValueError:
                continue
            raise ValueError(f"{name} must stay outside the immutable app directory")

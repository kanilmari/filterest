...
export function resolveFilterestHomes(projectRoot, environment = process.env) {
  ...
  const nestedPublicInstall = (
    !privateSource && isNestedFilterestInstallation(normalizedProjectRoot)
  );
  const values = {
  ...
  const configPaths = ['filterest.paths', 'filterest.paths.local'].map(
    (configName) => path.join(normalizedProjectRoot, configName),
  );
  if (nestedPublicInstall) {
    configPaths.push(path.join(normalizedProjectRoot, 'config', 'filterest.paths'));
  }
  for (const configPath of configPaths) {
    const fileValues = readPathsFile(configPath);
    for (const key of [
      'projects_home',
      'keys_home',
      'runtime_data_home',
...
export function resolveEaselectPrivatePaths(
  ...
  if (
    !privateSource
    && !isNestedFilterestInstallation(homes.projectRoot)
    && !homes.keysHomeConfigured
  ) {
    return {

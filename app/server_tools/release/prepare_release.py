#!/usr/bin/env python3
"""Prepare one candidate's product metadata and browser bundle from reviewed source.
...
The minified browser bundle is one of those generated bytes. Preparation builds
it from the reviewed source into a temporary directory and stages the result
beside the metadata, so a candidate cannot carry a bundle older than the code it
ships and nobody has to remember to rebuild it by hand.
"""
...
# Directories whose whole content one preparation regenerates: a file that is
# no longer produced must disappear rather than linger beside its replacement.
REGENERATED_DIRECTORIES = (BUNDLE, browser_bundle.BUNDLE_DIRECTORY)
...
def prepare_notices(root, version, staging, dist_dir=None):
    ...
    browser, browser_source = notices.collect_browser_bundle_dependencies(root, dist_dir=dist_dir)
...
def snapshot_outputs(root, planned):
    """Capture existing bytes, including regenerated files removed by this candidate."""
    paths = set(planned)
    for relative_directory in REGENERATED_DIRECTORIES:
        directory = regular_path(root, relative_directory)
...
def prepare(args):
    ...
    with preparation_lock(root):
        ...
        with tempfile.TemporaryDirectory(prefix="filterest-release-preparation-") as directory:
            ...
            bundle = staging / "browser-bundle"
            planned.update({f"{browser_bundle.BUNDLE_DIRECTORY}/{name}": content
                            for name, content in browser_bundle.build_browser_bundle(root, bundle).items()})
            # The notices must describe the bundle this candidate ships, not the
            # one the checkout happened to carry into the preparation.
            planned.update(prepare_notices(root, version, staging / "notices", dist_dir=bundle))
...
def main(argv=None):
    ...
    try:
        result = prepare(args)
    except (OSError, ValueError, browser_bundle.BundleBuildError,
            notices.InventoryCollectionError, subprocess.SubprocessError) as error:

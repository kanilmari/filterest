...
_start_vite_dev() {
    ...
    if ! command -v npm &> /dev/null; then
        echo -e "${YELLOW}⚠️  npm not found — skipping Vite dev server${NC}"
        return 1
    fi
    ...
    _launch_detached /dev/null env \
        ...
        npm --prefix "$FILTEREST_BUILD_ROOT" run dev -- --host 127.0.0.1 --strictPort
    ...
    echo "   Start manually from ${FILTEREST_BUILD_ROOT}: VITE_DEV_PORT=${vite_port} VITE_HMR_PORT=${vite_hmr_port} npm run dev"
    return 1
}

...
init_instance() {
    ...
    if docker ps --format '{{.Names}}' 2>/dev/null | grep -q "easelect-${instance}-db"; then
        ...
        local backup_dbname="${DB_NAME:-$(project_default_db_name)}"
    ...
    if ! docker ps --format '{{.Names}}' 2>/dev/null | grep -q "easelect-${instance}-db"; then
        ...
        wait_for_instance_db "$instance" "${DB_ADMIN_USER:-admin_user}" "${DB_NAME:-$(project_default_db_name)}" 60
    fi

    # Read DB_ADMIN_USER from instance env
    source "$env_file"
    local db_admin="${DB_ADMIN_USER:-admin_user}"
    local db_name="${DB_NAME:-$(project_default_db_name)}"
...
sync_instance() {
    ...
    echo "   ⏳ Waiting for database..."
    if ! wait_for_instance_db "$instance" "${DB_ADMIN_USER}" "${DB_NAME:-$(project_default_db_name)}" 60; then
...
no_sync_tables = set()
sync_config_file = os.environ.get('FILTEREST_SYNC_CONFIG_FILE', '')
if sync_config_file:
    try:

...
start_instance() {
    ...
    if [[ ! -f "$env_file" ]]; then
        ...
        local template_file="${FILTEREST_INSTANCE_TEMPLATE_PATH:-}"
        if [[ -n "$template_file" && -f "$template_file" ]]; then
            echo "   2. cp ${template_file} ${env_file}"
        else
            echo "   2. Create ${env_file} from the deployment environment example"
        fi
    ...
    if [[ "$env_type" == "prod" ]] && [[ -f "$docker_root/docker-compose.traefik.yml" ]]; then
        echo "   Using Traefik reverse proxy (production mode)"
        local project=$(compose_project_name "$instance")
        ccmd="docker compose -p ${project} -f ${docker_root}/docker-compose.traefik.yml -f ${docker_root}/docker-compose.instance.yml --env-file ${env_file}"
    else

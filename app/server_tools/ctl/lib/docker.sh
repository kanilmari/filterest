...
_local_docker_compose() {
    ...
    docker compose --env-file "$EASELECT_RUNTIME_ENV_FILE" -f "$compose_file" "$@"
}
...
start_docker() {
    # The portable public checkout owns a dedicated Docker lifecycle command
    # that creates its secrets and validates the standalone stack. Easelect's
    # outer wrapper supplies a private Compose file and continues through the
    # development/restore workflow below.
    if filterest_is_standalone_public_install; then
        if [[ "$RESTORE_DB" == true ]]; then
            echo "error: --restore-db belongs to the embedded Easelect development stack" >&2
            echo "Use ./filterest docker start for portable Filterest startup." >&2
            exit 2
        fi
    ...
    if [[ "$RESTORE_DB" == true ]]; then
        ...
        if [[ -n "$existing_public_tables" && "$existing_public_tables" != "0" ]]; then
            ...
            echo "   docker compose -f $(filterest_local_docker_compose_file) down -v"
            exit 1
        fi

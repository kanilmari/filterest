...
read_seed_db_config() {
    ...
    seed_name="${seed_name:-$(project_default_db_name)}"
}
...
# Resolve optional multi-instance definitions supplied by an embedding product.
# Standalone Filterest does not require these files for its own Docker stack.
instance_docker_root() {
    printf '%s\n' "${FILTEREST_INSTANCE_DOCKER_ROOT:-${PROJECT_ROOT}/docker}"
}
...
compose_files_for_instance() {
    ...
    local db_port=""
    local docker_root
    docker_root="$(instance_docker_root)"

    printf '%s' "-f ${docker_root}/docker-compose.instance.yml"

    db_port=$(grep -E "^DB_PORT=" "$env_file" 2>/dev/null | tail -1 | cut -d'=' -f2)
    if [[ "${db_port}" == "0" ]]; then
        printf ' %s' "-f ${docker_root}/docker-compose.instance.no-db-port.yml"
    fi
}

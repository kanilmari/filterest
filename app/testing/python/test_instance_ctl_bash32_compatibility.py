...
class InstanceCtlBash32CompatibilityTests(unittest.TestCase):
    def test_case_insensitive_instance_resolution_uses_portable_lowercase(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = run_system_bash(
                f'source "{PUBLIC_SOURCE_ROOT}/server_tools/ctl/lib/common.sh"\n'
    ...
    def test_instance_list_normalizes_production_under_system_bash(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = run_system_bash(
                f'source "{PUBLIC_SOURCE_ROOT}/server_tools/ctl/lib/common.sh"\n'
                f'source "{PUBLIC_SOURCE_ROOT}/server_tools/ctl/lib/instance_helpers.sh"\n'
                f'source "{PUBLIC_SOURCE_ROOT}/server_tools/ctl/lib/instance_list.sh"\n'
                'docker() { return 1; }\n'
                'curl() { return 1; }\n'
                "list_instances",
                cwd=temp_root,
    ...
    def test_instance_backup_policy_preserves_defaults_without_duplicates(self) -> None:
        result = run_system_bash(
            'BLUE=""; YELLOW=""; NC=""\n'
            f'source "{PUBLIC_SOURCE_ROOT}/server_tools/lib/sql_dump_policy.sh"\n'
            f'source "{PUBLIC_SOURCE_ROOT}/server_tools/ctl/lib/instance_backup.sh"\n'
    ...
    def test_instance_retirement_uses_protected_trash_and_keeps_named_volumes(self) -> None:
        ...
        backup_source = (
            PUBLIC_SOURCE_ROOT / "server_tools" / "ctl" / "lib" / "instance_backup.sh"
        ).read_text(encoding="utf-8")
    ...
    def test_instance_retirement_preserves_active_and_deleted_media(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = run_system_bash(
                ...
                f'source "{PUBLIC_SOURCE_ROOT}/server_tools/ctl/lib/instance_crud.sh"\n'
                'docker() { return 0; }\n'
    ...
    def test_management_instance_config_needs_no_in_place_sed(self) -> None:
        ...
        self.assertEqual(result.stdout, "1|1|1|DB_NAME=easelect\n")

    def test_standalone_instance_config_uses_filterest_database_default(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir)
            result = run_system_bash(
                'RED=""; GREEN=""; BLUE=""; NC=""\n'
                f'FILTEREST_SOURCE_ROOT="{PUBLIC_SOURCE_ROOT}"\n'
                'export FILTEREST_SOURCE_ROOT\n'
                f'source "{PUBLIC_SOURCE_ROOT}/server_tools/ctl/lib/common.sh"\n'
                f'source "{PUBLIC_SOURCE_ROOT}/server_tools/ctl/lib/instance_helpers.sh"\n'
                f'source "{PUBLIC_SOURCE_ROOT}/server_tools/ctl/lib/instance_crud.sh"\n'
                '_next_instance_slot() { printf "4\\n"; }\n'
                'create_instance public-demo demo.test application >/dev/null\n'
                'grep "^DB_NAME=" "instances/public-demo/.env"',
                cwd=temp_root,
            )

        self.assertEqual(result.stdout, "DB_NAME=filterest\n")

    def test_instance_shell_sources_have_no_known_bash4_or_gnu_only_constructs(self) -> None:
        source_paths = list((PUBLIC_SOURCE_ROOT / "server_tools" / "ctl").rglob("*.sh"))
        source_paths.extend((PUBLIC_SOURCE_ROOT / "server_tools" / "ctl").rglob("*.bash"))
        source_paths.append(PUBLIC_SOURCE_ROOT / "server_tools" / "lib" / "sql_dump_policy.sh")
        source_paths.append(PUBLIC_SOURCE_ROOT / "server_tools" / "lib" / "toolchain_version.sh")
        executable_source = "\n".join(
            line

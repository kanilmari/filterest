...
PRODUCT_ROOT = PROJECT_ROOT.parent if PROJECT_ROOT.name == "app" else PROJECT_ROOT
...
CONFIG_PATH = PROJECT_ROOT / "docs/constitution/design/technical_definitions.md"
PRINCIPLES_PATH = PROJECT_ROOT / "docs/constitution/design/README.md"


def resolve_test_runtime_root():
    """Resolve mutable test output outside the maintained application source."""
    configured_root = os.getenv("FILTEREST_TEST_RUNTIME_ROOT", "").strip()
    runtime_root = (
        Path(configured_root).expanduser()
        if configured_root
        else PRODUCT_ROOT / "data/testing"
    )
    if not runtime_root.is_absolute():
        runtime_root = PRODUCT_ROOT / runtime_root
    ...
    try:
        ...
        return runtime_root
    raise ValueError(
        "FILTEREST_TEST_RUNTIME_ROOT must resolve outside the immutable app directory: "
        f"{PROJECT_ROOT}"
    )


TEST_RUNTIME_ROOT = resolve_test_runtime_root()
SCREENSHOTS_DIRS = [
    TEST_RUNTIME_ROOT / "test-results/visual_guardian",
    TEST_RUNTIME_ROOT / "my-test-results/visual_guardian",
]
REPORT_FILE = TEST_RUNTIME_ROOT / "test-results/visual_guardian/report.json"

...
This script keeps the recreated schema skeleton and dummy seed rows aligned with
one explicitly resolved schema inventory.
...
INSTALLATION_ROOT = (
    PROJECT_ROOT.parent
    if PROJECT_ROOT.name == "app" and (PROJECT_ROOT.parent / "compose.yml").is_file()
    else PROJECT_ROOT
)


def resolve_default_schema_info() -> Path:
    """Resolve the schema inventory from installation-owned runtime data."""

    return INSTALLATION_ROOT / "data/db_backups/schema_info.csv"


DEFAULT_SCHEMA_INFO = resolve_default_schema_info()


def resolve_generated_output_directory() -> Path:
    """Keep disposable fixture output in installation-owned runtime data."""

    return INSTALLATION_ROOT / "data/testing/essential/generated"


GENERATED_OUTPUT_DIRECTORY = resolve_generated_output_directory()
SCHEMA_OUT = GENERATED_OUTPUT_DIRECTORY / "schema.sql"
SEED_OUT = GENERATED_OUTPUT_DIRECTORY / "seed.sql"
            ...
            "rows": [
                {
                    ...
                    "url_route_endpoint": "/api/app/agent-tools/task-groups",
                    "ui_only": False,
                },
                {
                    "id": 2002,
        ...
        "system_config": {
            ...
            "rows": [
                ...
                },
                {
                    "id": 3002,
                    "key": "site_name",
                    "json_value": {"value": "Filterest Essentials"},
                    "created": "2026-03-29 00:00:00",
                    "updated": "2026-03-29 00:00:00",
                    "creation_spec": "fixture seed",
                    "boolean_value": None,
                    "text_value": "Filterest Essentials",
                    "int_value": None,
                    "value_type": 2,
                },
            ],
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Path to schema_info.csv (defaults to installation-owned data/db_backups/schema_info.csv).",
    )

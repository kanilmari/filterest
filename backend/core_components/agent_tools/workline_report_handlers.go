...
func createWorklineReportHandler(w http.ResponseWriter, r *http.Request) {
	...
	if worklineStatus == "archived" {
		httpresponse.RespondWithError(w, http.StatusConflict, "archived_workline_rejects_new_reports")
		return
	}
	if worklineStatus != normalized.WorklineStatusSnapshot {
		httpresponse.RespondWithError(w, http.StatusConflict, "workline_status_snapshot_is_stale")
		return
	}
	...
	err = tx.QueryRow(
		`INSERT INTO dev_agent_workline_reports (
			...
			source_ref, tags, metadata_json, content_hash, format_version, phase_gate,
			...
			$1, $2, $3, $4, $5, $6, $7, $8, $9::jsonb, $10, 2, $11,
			$12, $13, $14, $15, $16, $17, $18::jsonb, $19, $20, $21, $22, $23, $24
		)
		RETURNING id`,
		normalized.WorklineID,
		normalized.Title,
	...
	if _, err := tx.Exec("UPDATE dev_agent_worklines SET updated = now() WHERE id = $1", normalized.WorklineID); err != nil {

...
func createWorklineHandler(w http.ResponseWriter, r *http.Request) {
	...
	err = backend.Db.QueryRow(
		`INSERT INTO dev_agent_worklines (title, status, tags, created_by)
		 VALUES ($1, $2, $3, $4) RETURNING id`,
		title, status, pq.Array(tags), currentAgentToolUserID(r),
	).Scan(&id)
	if err != nil {
...
func updateWorklineHandler(w http.ResponseWriter, r *http.Request) {
	...
	var id int64
	if err := backend.Db.QueryRow(query, args...).Scan(&id); err == sql.ErrNoRows {
		httpresponse.RespondWithError(w, http.StatusNotFound, "workline_not_found")
		return
	} else if err != nil {

...
- **A server error is reported once, in the reader's language** — A failed request showed a hardcoded Finnish "Palvelinvirhe (500)" with the request address to everyone, and on top of the screen's own translated error when the screen already reported it. The notice now comes from language keys, keeps the address in the console, and stays silent when the calling screen shows the failure itself; a lost connection is reported the same way. The dataset form's folder hint shows its real guidance instead of a placeholder.
